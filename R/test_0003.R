...
t_scribbles <- ggplot2::layer_data(d_scribbles)
...
for (i in 1:lines) {
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_point(
      data = t_scribbles |> dplyr::filter(group == i),
      ggplot2::aes(x = x, y = y), # alpha = index
      shape = 16, colour = scribble_colour, size = 0.5) +
    ggplot2::geom_point(
      data = t |> dplyr::filter(group == i),
      ggplot2::aes(x = x, y = y, size = index / 2), # alpha = index
